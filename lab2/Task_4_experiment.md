...
This is why we decided to establish simultaneous communication between the leftmost (`h21`, `h22`) and the rightmost (`h35`, `h36`) pairs of hosts, measuring the throughput for TCP and UDP via `iperf`. For additional comparison we also measured the throughput for just one pair of hosts out of these two groups. More details on how we did that follow in the next section.
...
The experiment itself is executed twice (once for TCP and once for UDP) and consists of 2 Stages:
1. `h21` listens to `h35`
2. `h21` listens to `h35` and, at the same time, `h22` listens to `h36`
